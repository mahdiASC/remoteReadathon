    ...
    base('Table 2').select({
        ...
        if(holds.length>0){
            ...
            for(var i in holds){
                ...
                holdContent = holdContent + "<li>"+ ID + "<select id='"+ID+"' class='selectpicker' style='color:black;' data-style='btn-primary'><option>0</option><option>1</option><option>2</option></select>" + "<textarea class='form-control' id='comments"+ID+"' rows='1'></textarea>" +"<button onclick=\""+"updateHold('"+holds[i].id+"')\" style='vertical-align:middle'>Input Data (Removes hold)</button>"+"<button onclick=\""+"forceRead('"+ID+"')\" style='vertical-align:middle'>Read and Print</button>"+"</li>";
    }
...
function forceRead(studentID){
    $("#readStudent").val(studentID);
    readStudent();
}
