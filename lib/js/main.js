    ...
    base('Table 2').select({
        ...
        if(stats.holds.length>0){
            ...
            for(var i in stats.holds){
                ...
                holdContent = holdContent + "<li>"+ ID + "<button class='button' onclick=\""+"readHold('"+stats.holds[i].id+"')\" style='vertical-align:middle'>Read and Score</button></li>";
        }
...
function enterData(){
    //Clear before adding
    $("#paras").empty();
    var userScore = Number($("#userData").val());
...
function ticker(){
    ...
    base('Table 2').select({
        ...
        $("#readathonStat").text(completed.length + " out of "+ total+" TOTAL apps done = " + Math.round((completed.length/total)*100)/100+"% done!");
    });
}
