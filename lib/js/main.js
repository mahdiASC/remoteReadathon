...
let table_name = "Readathon"; // specific tab by name
let table_view = "Grid view"; // specific view by name

// the names of the columns for the essay questions
let essay_columns = [
...
Airtable.configure({
    ...
    apiKey: "keyVIWCOZULuFuVKd" // be sure the change this after!
});
...
function checkValidRecord(record){
    ...
    return !checkHold(record)&&!checkUserScored(record)&&!checkCompleteRecord(record)&&!checkAlumni(record)&&!checkFullReaders(record);
}

function checkFullReaders(record){
    //true if readers are full (3 or more users) /  HACK!
    return record.fields.readers ? record.fields.readers.split("+").length>2 : false; 
}

function checkAlumni(record){
    //returns true if alumni are present in the reader list or if user
    if(Number(userID)>899){
        if(record.fields.readers){
            return record.fields.readers.split("+").some((user)=>Number(user)>899);
        }else{
            return false;
        }
    }else{
...
function checkUserScored(record){
    ...
    return !!record.fields.readers ? record.fields.readers.split("+").includes(userID) : false;
}

function checkCompleteRecord(record){
    //returns true if a record if completely graded
    return record.fields.Score1!=undefined&&record.fields.Score2!=undefined&&record.fields.Score3!=undefined;
}
...
function getRandStudent(){
    ...
    base(table_name).select({
            ...
            if (err) { 
                ...
                var column;
                if(selectedStudent.fields.Score1==undefined){  
                    column = "Score1";
                }else if(selectedStudent.fields.Score2==undefined){
                    column = "Score2";
                }else{
                    column = "Score3";
                }
                ...
                base(table_name).update(selectedStudent.id, tempObj, function(err, record) {
                        if (err) { 
                            alert(err); 
                        }
...
function enterData(){
    ...
    base(table_name).find(selectedStudent.id, function(err, record) {
        ...
        //resetting student
        base(table_name).update(record.id, tempObj, function(err, record) {
                if (err) { 
                    alert(err);
                }else{
