...
function checkValidRecord(record){
    ...
    var alumn = Number(userID)>899 ? !(record.fields.Score1>899||record.fields.Score2>899||record.fields.Score3>899)&&!otherAlum: false;
    
    var fullRecord = record.fields.readers ? !record.fields.readers.split("+").length>2 : false; //true if records are full (3 or more users) /  HACK!
    return !checkHold(record)&&!checkUserScored(record)&&!checkCompleteRecord(record)&&!alumn&&!fullRecord;
}
